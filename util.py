...
class AwsConfig:
    ...
    def __post_init__(self):
        ...
        if self.file_format not in valid_file_formats:
            raise ValueError(f"Invalid file_format '{self.file_format}'. Must be one of {valid_file_formats}.")
        ...
        if self.mode not in {"overwrite", "append"}:
            raise ValueError(f"Invalid mode '{self.mode}'. Must be 'overwrite' or 'append'.")

        if self.partitioned_by and not self.partitions_types:
            raise ValueError("If 'partitioned_by' is specified, 'partitions_types' must also be provided.")
